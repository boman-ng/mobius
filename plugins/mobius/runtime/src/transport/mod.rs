...
const USAGE: &str = "Mobius v1.0.0\n\nUsage:\n  mobius mcp\n  mobius read ...\n  mobius audit ...\n  mobius doctor ...\n  mobius report ...\n  mobius hook session-start\n  mobius hook pre-tool-use\n  mobius hook stop\n";
...
pub(crate) fn run(arguments: impl Iterator<Item = OsString>) -> Result<(), MobiusError> {
    ...
    match mode {
        ...
        Mode::Read | Mode::Audit | Mode::Doctor | Mode::Report => cli::run(first, tail),
...
mod tests {
    ...
    fn mode_dispatch_is_closed_over_the_blueprint_surface() {
        let accepted = ["mcp", "read", "audit", "doctor", "report", "hook"];
