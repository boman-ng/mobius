...
Mobius v1.1.0 is the current stable single-binary release. The checked-in marketplace deliberately
...
```bash
sha256sum --check mobius-1.1.0-x86_64-unknown-linux-gnu.tar.gz.sha256
tar -xzf mobius-1.1.0-x86_64-unknown-linux-gnu.tar.gz
codex plugin marketplace add ./mobius-1.1.0-x86_64-unknown-linux-gnu
...
installed plugin cache. Review and trust the packaged Hooks before using Mobius on a project. On
the first trusted startup, Mobius asks the main Agent once to inspect any locally available
CC-Switch models and configure the user-level `mobius-judge` custom agent when there is one clear,
compatible choice. Later Sessions reuse Codex's global configuration without repeating discovery;
a newly created custom agent becomes available after starting another thread.
