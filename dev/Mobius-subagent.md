...
以下矩阵是 Mobius 原生角色的 execution policy：

| Role         | model 与 effort                              | 选择理由                               |
| ------------ | -------------------------------------------- | -------------------------------------- |
| `scout`      | GPT-5.6 Luna / `medium`                      | 清晰、可重复的本地调查与结构化摘要     |
| `researcher` | GPT-5.6 Terra / `medium`                     | 边界明确的开放世界检索、来源比较与综合 |
| `driver`     | 继承 Main Agent 的 model 与 reasoning effort | 延续 Main Agent 的有界执行上下文       |
| `verifier`   | GPT-5.6 Luna / `high`                        | 对明确 claims 和 checks 做可重复验证   |
| `judge`      | 默认 GPT-5.6 Sol / `medium`                  | 复杂、开放式、反证优先的语义判断       |

该矩阵适用于默认 native spawn。main Agent 在调用前确认所选 model 与 effort 可用；host 不能提供精确配置时按
配置失败终止，不替换 model、不降低或提升 effort。Driver 不设置专属覆盖，继承 Main Agent 的 model 与
reasoning effort。下述不同模型族 Judge 是唯一 model-selection 例外：它使用 Host 配置的 model 与 effort，
但不改变 `judge` 角色或合同。

审查需要不同模型族的独立视角时，main Agent 使用 Host 已配置的 `mobius-judge` custom agent 启动同一个
`judge` 角色；task envelope、Judge role profile、freeze gate 与结果合同保持不变。Runtime 返回的实际 agent、
model 与 provider 只是执行事实。Runtime 未能证明实际模型族不同于 GPT-5.6 时，该独立视角要求未满足；不能从
custom-agent name 或 provider 推断模型族。该 custom agent 无法启动时如实报告配置或 Runtime 失败，不用默认
Judge 替代。
...
2. Driver 继承 Main Agent 的 model 与 reasoning effort，不设置 Driver 专属 model、effort、provider、sandbox 或 approval 覆盖；
...
需要多个模型视角时，main Agent 用相同的冻结材料、questions、criteria 和结果合同为每个 Judge 创建独立
任务并分别消费输出。票数、模型数量、Runtime success 或 Judge 推荐都不能替代 main Agent 的语义判断。
...
- Driver 继承 Main Agent 的 model 与 reasoning effort；Runtime 应用官方定义的 sandbox 与 permission 继承。
- `mobius-judge` custom agent 由插件的一次性 Host onboarding 配置；本 Skill 只在需要不同模型族视角时
  消费这个已配置 agent，不发现 provider、不修改用户配置，也不复制其运行时事实。
...
- provider 与 custom-agent provisioning 只由插件的一次性 Host onboarding 负责，不增加 setup Skill；
...
5. 每次启动都包含 background、一个或多个 objectives、forbidden-first boundaries、完整 output format 和
   DONE conditions，并与所选角色的 input/output format 组合。
6. 默认不提供 `allowed`；focus、roots 和已列路径不限制相关自主探索。只有重大风险场景才增加穷举式
   positive allowlist。
7. 模型选择矩阵与五类角色的输入输出模板完整保留；只有一个 Judge 合同，默认使用 GPT-5.6 Sol / `medium`，
   不同模型族只改变本次 spawn 的 Runtime 配置，不产生新的角色、路由或结果类型。
...
13. 模型 policy 随官方能力演进；skill 不固定 Runtime 内部实现，也不以 per-thread tool hiding 或 caller
