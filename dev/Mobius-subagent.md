...
角色是否需要由 caller-owned workflow gate 决定；本通用合同既不规定角色配额，也不否定 caller 对某次流程的
required role。Driver 适合 write set、授权、baseline、effects 与 validations 都明确的一项变更；高度耦合、
立即阻塞 main Agent 下一步或授权不清时由 main Agent 直接工作。Verifier 适合实现路径之外的复现、故障注入和
高风险边界检查，只在被验证 effect 已发生并稳定后启动，且不修复 subject。caller 请求 Judge 时，材料必须先
冻结并声明 required coverage；Judge 始终只返回 advice。按风险标签机械映射 role、固定额外人数/顺序或投票不
属于本通用合同。角色不可用或 coverage 不足时如实返回 unavailable、degraded、partial 或 inconclusive，由
caller policy 决定后续动作。
...
| `judge` external | 当前 Runtime 已公开的 external Judge agent profile / profile 自有配置 | 在相同 Judge 语义下提供不同模型族视角 |
...
当前可用配置，但该 closest-configuration 路径不适用于 external Judge。Driver 不固定专属 model 或 effort；
实际选择由 Codex Runtime 解析。

输入很小、检查机械且反例空间有限时，可以降低 effort；研究、验证或审查变得复杂、开放、跨域或高影响
时，应升级到更强的当前可用模型或 effort。偏离矩阵不改变角色语义。

### 3.3 External Judge profile 的发现与选择

只有尚未覆盖的审查问题能从独立模型族视角获得实质新增信息时，main Agent 才考虑 external Judge。候选必须是
当前 Runtime 已公开为 external 或 independent、且其公开 purpose 与 capabilities 适合只读 Judge 任务的原生
agent profile。Runtime profile identity 与公开描述只是选择时事实；main Agent 不按 profile 名称或未公开的
model identity 推断 externality。

若 Runtime 未公开合格 external profile，main Agent 如实报告 external Judge unavailable。内部模型仍可承担
internal Judge，但不得被标记、计数或报告为 external Judge。main Agent 不从任何 filesystem path 或
configuration file 发现或解析 external profile，包括 `~/.codex/agents`；不枚举 provider model、不维护
profile registry，也不直接调用 provider。Skill 不固定任何 provider、model 或 profile 名称。

通过独立视角 gate 后默认只选一个 external Judge。需要多个时，main Agent 在 spawn 其中任何成员前完整声明
一个有限 profile 集合，为每个 profile 分配一个互不重复且尚未覆盖的问题、失效模型或反证面，并把一个有限
总 result budget 拆成各 child budget；没有预期新增信息价值的 Judge 不启动。main Agent 只按 Runtime identity
请求 profile，provider、model、effort、sandbox 与 permission 由 Runtime 应用。

Runtime advertisement 只证明选择时可见；spawn 是实际 availability 的权威检查。profile 缺失、identity 改变或
无法解析、spawn 失败、timeout、interruption、invalid result 或缺失 final output 时，该次 external Judge 标记为
unavailable 或 degraded，并在 Judge result envelope 之外如实报告，由 caller policy 决定能否缺少该 advice 继续。
不得用 closest model、internal profile、另一 transport 或伪造成功结果替代 external Judge。

### 3.4 Driver 的唯一执行路径：host 原生 Subagent workflow
...
所有模板中列出的 status/assessment/severity alternative 都是闭合枚举。consumer 必须按唯一 ID 闭合每个
material、question、criterion、known risk 及其 result；finding 的 criterion/material/evidence 与
recommendation、通用 envelope 的 artifact/evidence 引用必须解析到当前 task-local inventory。
missing、extra、duplicate、unknown 或跨 inventory 引用使结果无效，不能被 prose、finding 或 disposition 修复。
...
需要多个模型视角时，main Agent 遵守第 3.3 节的 profile gate，先预声明有限 Judge 集合与总 result budget，
并让每个新增 Judge 覆盖尚未覆盖的具体问题、失效模型或反证面。各 Judge 独立返回，main Agent 只生成一份去重综合；重复的全文、材料清单与发现通过
...
- External Judge 只从 Runtime 当前公开的合格 external profile 中选择；没有合格 profile 时 unavailable，不能
  用 internal model、closest model、另一 transport 或伪造结果代演 external Judge。
...
6. 默认不提供 `allowed`；focus、roots 和已列路径不限制相关自主探索。只有重大风险场景才增加穷举式
   positive allowlist。
7. 模型选择矩阵与五类角色的输入输出模板完整保留，并作为语义指导而非自定义传输协议；external Judge
   只使用 Runtime 当前公开的合格 external profile，无候选时如实 unavailable，internal model 不能代演。
...
13. result envelope 整体遵守序列化 byte budget、去重并显式报告 overflow；单个 item 不能绕过上限，
    大型或原始内容只以可核查 artifact locator 返回。Judge fanout 在 spawn 前预声明有限集合，覆盖不同问题
    或失效模型并共享有限总预算，main Agent 只生成一份综合。
