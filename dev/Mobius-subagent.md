...
| `judge` internal | `gpt-5.6-sol` / `medium`                                    | 复杂、开放式、反证优先的语义判断       |
| `judge` external | 一个已可用的外部模型 / 其支持的 effort                      | 在相同 Judge 语义下提供不同模型族视角  |

Sol、Terra 和 Luna 的工作负载定位依据当前官方
[Codex Models](https://learn.chatgpt.com/docs/models#choosing-sol-terra-and-luna)。main Agent 在调用前确认
显式请求的 model 和 effort 是否可用；不可用时选择最接近该工作负载的当前可用配置。Driver 不固定专属
model 或 effort；实际选择由 Codex Runtime 解析。
...
需要多个模型视角时，main Agent 为每个 Judge 创建独立任务并分别消费输出。票数、模型数量、Runtime
success 或 Judge 推荐都不能替代 main Agent 的语义判断。
...
`mobius-subagent` 应保持为显式触发、instruction-first 的薄 skill：
...
- 普通 Codex 任务不隐式进入本委托工作流，只有用户或 main Agent 显式选择时才触发本 skill。
