...
| Main Agent Composition | Driver/Verifier 选择、每 Stage Review 的 required Judge、delegation baseline、Runtime 结果核查、effect 处理、候选转义、正式 Judgment、人类确认协调与 Core 提交 |
...
- Model Core 可以在不认识或调用 Subagent Skill 的情况下独立运行与测试；符合 Mobius Loop 的 Stage
  `accept` 路径必须在 Composition 中完成 required Judge 仪式；
...
4. 用一个 Rust 单二进制运行时承载 Core service、最小 MCP、无业务状态 mutation authority 的 CLI、hook
   handler 与维护逻辑；
5. 用独立薄 skill 提供通用 subagent 委托能力，由 Composition 按信息价值选择 Driver/Verifier，并在每个
   Stage Review 固定调用 Judge；
...
#### 4.1.1 Evidence Bundle v1

依赖可变工作成果、命令输出或外部对象的 `stage_review` observation 使用 canonical Evidence Bundle，并优先把
完整 canonical bytes 作为现有 `CoreSnapshot` 捕获。只有完全自包含、足够小且不引用可变外部位置的 intrinsic
观察可以使用 `Inline`。Bundle 是 observation bytes，不是 Core object、Trail 之外的 ledger 或外部世界 resolver。
...
material_baseline:
  ...
  scope:
    include: [finite normalized project-relative locator]
    exclude: [finite normalized project-relative locator]
  before: {}
  after: {}
verification:
  - check_id: task-local unique id
    ...
    output_identity: sha256 content identity
...
`verification` 非空。所有 SHA-256 identity 使用 `sha256:` 加 64 个小写十六进制字符。Locator 必须是去重排序的
UTF-8 project-relative 规范路径，不含空段、`.`、`..`、NUL 或绝对路径。Object key 按 UTF-8 bytes 排序；
set-like array 排序去重；`verification` 保留有语义的执行顺序。Canonical JSON 只允许 null、boolean、integer、
string、array 与 object，无浮点、无非必要空白。Bundle 不含时间戳、绝对个人路径、secret、无关日志或自引用
digest，canonical bytes 上限固定为 131072 bytes；超限候选保持模型外并缩小 observation scope，不能拆成多个
会改变原子语义的 Bundle。

完整 64-hex identity 只在 capture、artifact reference、Bundle bytes、freeze declaration、MCP/Core input、
Trail/SQLite 与实际 equality/integrity check 等机器边界出现。进入 Agent current context 后，为每个已全值核对的
identity 分配 task-local semantic id（如 `B1`、`E3`、`JM1`）；后续引用优先只传 local id。需要给人显示 hash
线索时使用 `sha256:…<末 7 hex>`，它只是 display hint，不参与 equality、lookup、freeze 或 admission。当前有限
集合中若 suffix 冲突，扩展到足以区分的尾部长度或只显示 local id；不得猜测 short suffix 对应的 full digest。
Delegation 只在 material freeze 首次声明时携带一次 full digest，result 用 material id 引用，避免重复注入。

各 baseline kind 的 `before`/`after` 使用同一 variant：

- `repository_worktree`：同一 capture version、scope、exclusions 与 toolchain input 下的 `base_tree`、
  `tracked_delta_digest`、`untracked_manifest_digest`、`toolchain_config_digest` 全部必填；
- `artifact_set`：非空、按唯一 `logical_id` 排序的 `digest + size_bytes` entries；
- `external_object_set`：非空、按唯一 `logical_id` 排序，每项固定 `content_digest` 或稳定
  `immutable_version`/等价 immutable identity；可变 locator 不能单独成为 identity；
- `intrinsic`：`before={}`、`after={}`，并固定完整 observation identity，不引用可变位置。

`repository_worktree` capture v1 只接受 canonical project root 内有限 scope。默认排除 `.mobius`、`.tmp` 与已解析、
以精确 project-relative locator 记录的 managed Cargo target；不得按任意名为 `target` 的路径段排除。无法确认
managed target locator 时 capture 失败。普通 `src/target` 等路径仍属于材料，除非它本身就是已解析的 generated
target。
`base_tree` 使用当前 `HEAD^{tree}` 与 repository object format；unborn/non-Git workspace 改用 `artifact_set`。
tracked delta 对 HEAD 到 index+worktree 的完整 binary diff bytes 求 SHA-256，固定 C locale、no-color、
no-ext-diff、full-index、no-renames 与 literal pathspec。Untracked manifest 从 `exclude-standard` 的 scope 结果生成，
按 relative path 排序；regular file 记录 path/kind/size/content digest，symlink 记录 link-target bytes digest且不跟随。
非 UTF-8 path、非 regular/symlink、containment failure 或无法确认 managed exclusion 时 capture 失败。
Toolchain manifest 对 Route 声明的 lockfile、build config、feature set 与 tool-version entries 使用同一排序编码；
空集合使用固定 empty-manifest digest。

Reference evaluator 与 capture fixtures 只能存在于 Rust test surface，版本必须与上述 schema 完全一致；它可被
deterministic fixture 与 clean-host E2E 复用，但不得进入 domain、reducer、Trail、SQLite、application admission、
MCP/CLI/hook surface 或 release binary。未知 schema/canonicalization 返回 unsupported/invalid，不猜兼容。

#### 4.1.2 Freshness 与 applicability gates

Composition 使用纯判定：

```text
valid(bundle) = common_fields_valid AND kind_fields_valid
coherent(bundle) = valid AND (kind = intrinsic OR canonical(before) = canonical(after))
current_applicable(bundle, now) =
  coherent AND (kind = intrinsic OR canonical(after) = canonical(capture_same_scope(now)))
```

三个 gate 依次执行：

1. `RecordEvidence` 前 capture before，执行验证，再 capture after；非 coherent 候选不提交，重新验证；validator
   通过后才 capture canonical Bundle bytes 并构造 Evidence。
2. `SealAttempt` 前重新 capture 同 scope。旧 Evidence 已 superseded 时保持 Attempting，重新验证并追加一条
   current-applicable Evidence。旧 Evidence 不修改；Core 仍把完整 `U_q` 物化进 Packet。
3. `Decision` 前完成 exact identity/artifact closure 后，在同一 decision fence 捕获 now，把每条 Bundle 分为
   current-applicable、superseded、unverifiable。依赖可变材料的 Criterion 至少需要一条覆盖它的当前 supports；
   当前 contradicts/unknown 必须在 findings 处置，否则选择 retry、replace、wait 或 remap，不能 accept。

这些 gate 缩小 check-to-submit race，但只证明冻结版本上的结论，不声称外部世界永久不变。Core 不读取 Git、
workspace、toolchain 或 remote object，也不伪装成能够验证开放世界 freshness。
...
→ 可选：使用 Subagent Skill 调查、执行或验证
→ main Agent 消费 Runtime result
→ 核查 baseline、freeze、effects、provenance、coverage、unknowns 与 cleanup
→ main Agent 构造完整 Model object 或 `SealAttempt` command
→ `SealAttempt` 时 Core 唯一物化 Packet；其他非 Review command 直接提供 Model input
→ Reviewing：递归闭合并冻结材料，执行一次 required Judge，main Agent 消费其 advice
→ main Agent 独立构造正式 Judgment
...
每个 Stage 进入 `Reviewing` 后，main Agent 必须先完成 exact recursive closure、artifact integrity、Evidence
applicability 与材料冻结，再通过通用 `mobius-subagent` 合同创建一个 fresh required Judge task。该 task 固定
本次材料、questions、criteria、known risks、disposition options 与 required coverage。每个 Review attempt
只有一个当前 required Judge slot；Packet、material、baseline 或问题变化时创建新任务，旧结果只保留为线索。
额外 Judge 只在覆盖不同问题、failure model 或 counterargument 时按信息价值启动。

main Agent 检查 required Judge 的 native final identity/status/output、完整通用 result envelope、唯一 Judge
`role_output`、所有 task-local reference、freeze match、required coverage、findings、uncertainties、overflow 与
budget。缺席、spawn/permission/Runtime failure、timeout、invalid output、stale、partial、unverifiable 或
inconclusive 结果不能填充 required slot，并阻止 `Decision(accept)`。有效结果也只是必要非充分条件；未被 main
Agent 以当前 Evidence 明确解决的 Judge objection 同样阻止 `accept`。
...
Subagent timeout、failure、boundary violation、inconclusive Judge 或 unavailable material 不自动推动 Model
状态。required Judge 不可用时 `accept` 停止；main Agent 保持 `Reviewing`，或在语义上成立时提交 retry、replace、
wait 或 remap。已发生 effect 不会因结果被拒绝而消失；main Agent 仍负责检查与 cleanup。

### 8.6 Stage Risk Card 与角色深度

Loop 在 `Ready → StartAttempt` 前建立一次 ephemeral Stage Risk Card，至少记录 effect 类型，migration/storage、
concurrency/lease/locking、filesystem/path/symlink/permissions、security boundary、crash/recovery/durability、
cross-stage integration、unknown/weak-testability 风险，Driver/Verifier 的 use/skip 理由，required Judge 的
pending/current/unusable 状态、Evidence independence/coverage 方法和待捕获 baseline。新 effect、风险或
baseline 改变选择时才更新；不持久化。

Driver 只在有限 write set、授权、baseline、effects 与 validations 都明确且能降低主线程负担时使用。Verifier 在
相关 effect 发生并稳定后，优先用于 migration、lease/concurrency、filesystem/security、crash、外部 effect 与
跨 owner integration；它不能修复 subject。Judge 在每个 Stage Review 都执行一次：只在 Packet recursive
closure、artifact integrity、applicability 与材料冻结完成后启动。额外 Judge 才按尚未覆盖的问题或独立视角的
新增信息价值选择。

验收 gate 绑定主要 failure model 的独立方法覆盖，而不绑定角色。独立 fixture、fault injection、CI、comparison
implementation 或不同机制的 direct reproduction 都可以提供 coverage。只有 accepted verification rule 或人类
在 Attempt 前批准的风险政策可以要求额外 role。不得要求每 Attempt 有 Driver、按风险名称机械映射 role、固定
额外人数/顺序或投票。Driver/Verifier 都可缺席，但不存在 Judge-free Composition accept path；方法不足或
required Judge 不可用时保持 Criterion 未解决或选择合法 non-accept outcome。

所有 native final result 依次检查：Runtime identity/status/final output、唯一完整公共 envelope 与唯一 role_output、
objective/assumption/DONE/boundary closure、effects/cleanup/artifacts/uncertainties/blockers/overflow、有限 result
budget、baseline/material freshness 与 role coverage。Runtime `completed`、模板存在或 summary 非空都不等于有效。
Judge 还执行第 7.1 节 matched-freeze/complete-coverage gate；external 只来自 Runtime 明确 advertised qualifying
profile。缺席、unavailable、degraded、stale、partial 或 inconclusive 不合成成功结果，也不替 main Agent Review。
对 required Judge，这些状态还会阻止 `accept`；只有 current、完整、freeze matched、coverage complete 的结果能
通过 ritual gate，且未解决 findings 仍阻止 `accept`。

### 8.7 Ephemeral proof-impact gate

每次 material effect 稳定后，main Agent 在当前上下文建立 `accepted Decision → owning Stage → frozen scope/
baseline → changed surface → disposition` matrix。它不写数据库或报告。Disposition 规则为：

- scope 与工具链/配置/依赖可证明无影响：`unaffected`；
- 与历史 scope、global config、migration、lease/concurrency、security 或共享 filesystem 相交：
  `needs_reverification`；
- Stage contract、dependency topology、Criterion ownership 或验收认知变化：`requires_remap`；
- scope/identity/外部版本不足以证明：`unknown`，在 Navigating 中按 fail-closed Remap。

只影响 current Stage 时留在 Attempting，重新验证并追加 Bundle。影响 accepted proof 时从当前 Navigating 提交
`RequestRemap`，安装新 Map，对受影响 Stage 及传递依赖给出 `invalid` carry 并重新执行；结构认知变化同时修订
Map。无影响不制造 Remap。Objective 终态后只报告 finding 或请求用户创建新 Objective，不提交旧流 transition。
...
#### 9.1.1 C0 Agent control contract

每个 Composition skill 为自己可能提交的 transition 提供 canonical draft reference。每个 entry 固定 externally
tagged command name、按 live MCP schema 顺序排列的 required fields 与恰好同字段的 payload template；outer
wrapper 统一为 project root/id、两级 heads、request id 与一个 command。测试必须通过真实 stdio MCP `tools/list`
读取 schema 并机械比较全部十二种 command，不能复制一份独立的 schema 作为通过依据。

Cockpit 固定包含 Objective、typed state、两级 heads、current subject、`Interaction(exact_path|unavailable)`、
Risk、Delegation、Evidence baseline、Proof impact、last accepted fact、next legal action 与 Fence。字段只从当前
supported read、exact object 或新构造的 typed object 提取，不能从 prose、错误消息、旧 cockpit、其他 Attempt
或 remembered heads 补值。Review closure 与 Wait batch 用 current-context-only ledger 记录 expected/read/
kind-id-integrity/status；不得持久化或成为事实源。

任何 read、hook、path、schema 或 binding failure 都令 `State=unknown`、`Fence=closed`。先运行 standalone
discovery/doctor；无法恢复就报告 owning failure且不 mutation。`invalid_tool_input` 丢弃完整 payload 与 request id，
从 canonical entry 重建；stale head 还丢弃 semantic decision、confirmation、closure/batch 与 fence。禁止 unchanged
retry 或只替换 heads。StructuralContext、AcceptanceContext、Packet、Decision 与 Wait identity 只能从当前 exact
typed object/state 复制。

Copilot 仅在显式 activation 且 binding 明确 `absent` 时调用一次 `project_init`；receipt 已知后当前 entry 不再
调用。Idempotency 只处理 response loss/crash，不是正常控制流。Compaction 后重读 binding，不因“可能失败”重复
初始化。

Activate/Revise/Abandon preview 显示 project/Objective、action、两级 heads、完整 canonical typed payload、
`sha256:…<末 7 hex>` payload digest display 与简短解释。Short digest 只降噪，不是 identity 或认证；Core 仍比较
complete typed confirmation。Payload、heads、action、Objective/project 或 intervening fact-changing action
变化都会使确认失效。

Accepted `interaction_path` 进入 cockpit/handoff 的 `exact_path`；丢失时为 `unavailable`。后续仍按完整 Objective/
revision 精确匹配，零个或多个结果都不猜。Compaction 只丢弃未验证 path，accepted handoff 要么重核对为 exact，
要么显式 unavailable，不进入 Trail、Map、Evidence、Subagent envelope 或新索引。

进度只在 skill entry/re-entry、accepted transition、risk/delegation decision、baseline/proof-impact 改变路径、
blocker/degraded advice 与 verified completion 更新。普通 read、无变化 wait、内部 parse 与 idempotent receipt 不单独
播报；超过 60 秒的更新只报告 state/risk/next legal action。

#### 9.1.2 C1 conditional read surface

C0 后必须先运行十个 synthetic fault fixtures、至少十个 allowlist-redacted representative sessions、至少一百个
transition drafts且每种 transition 至少一次。最小 corpus 未满足时 C1=`not_evaluated`，不增加 inspect、generic
query、MCP read tool、CLI view 或第二 read path。达到任一量化 trigger 也只能先提出 ADR，并正式修改第 4.7、
5.1、5.2、9.1 与 10.6 节后才允许代码工作。当前仓库没有获授权的十 session/百 draft corpus，因此 C1 不启动；
十个 deterministic fixtures 只证明 C0 fail-closed contract。

#### 9.1.3 D0 session audit contract

D0 只定义显式用户触发的诊断 input、完全合成 fixture 与 manual recipe。Input 固定 schema、tested adapter id、
source version/fingerprint、授权 session locator、exact Objective、allowlist redaction profile，以及 input bytes/
records/agents/result bytes 四个正 bounds。Trail 独占业务事实；Runtime export 只支持 tool/Subagent lifecycle 与
native final-output observation。两者只按 request id、typed identity、receipt/event digest 精确关联，歧义为 unknown。
Judge 缺席记 `absent`，不补造 freeze、coverage 或 disposition。Audit 必须把 Core/Trail lifecycle 与 Agent-path
conformance 分开：Trail 可重放到 `Achieved` 且 Core audit healthy，同时因任一 Stage Review 缺少 required Judge
而把 Agent path 标为 `nonconformant`，逐 Stage 输出 deviation 与聚合 residual risk。

Adapter 一次 streaming parse，不复制 transcript、保存 index/registry/memory/checkpoint 或建立 Runtime mirror。
Unknown adapter/schema mismatch、超 bounds、truncation、parse failure 或缺 event kind 使整体 degraded，并停止相关
结论。Redaction 默认删除消息/prompt/raw args/output、绝对个人路径、环境、usage、secret 与无关 Objective record；
locator 只输出不可逆 digest/授权短标签。默认 stdout；用户指定文件时只写其普通临时目录，不写 `.mobius`，不被
Core/Skill/Review/completion 读回。

D1 只有 host 提供受支持 versioned export/明确 fingerprint 与授权，且 adapter/redaction/bounds/degraded/retention
tests 全部通过、一次调用不持久化 mirror、蓝图先接受其 diagnostic-only ownership 后才启动。当前条件不成立，
D1=`not_evaluated`，不实现 parser/renderer 或作 automated-session-audit release claim。
...
- Driver/Verifier 不可用：Model 状态保持不变，main Agent 可以直接工作、改用独立方法、等待或报告真实
  blocker；required Judge 不可用时还必须停止 `accept`；
- report 中途崩溃：业务状态不受影响；旧 current、未引用 generation 或无效 current 由下一次显式 report 检测并重建。

### 9.4 正常路径独立性

必须分别验证一条 Core primitive path 与两条完整 Composition E2E：

```text
Human → Main Agent → Model Core
```

该路径只证明 Model Core、MCP、Trail 与 audit 不依赖 Subagent schema；它不是符合 Mobius Stage Review 仪式的
完整 Agent path。

完整 Composition 的两条正常路径是：

```text
Human → Main Agent performs Stage work
      → required Judge → Main Agent ReviewDecision → Model Core

Human → Main Agent → optional Driver/Verifier
                   → Main Agent translation
                   → required Judge → Main Agent ReviewDecision → Model Core
```

Core primitive path 证明 Subagent 不是 Model 的程序依赖；两条 Composition path 证明 Driver/Verifier 可选、
required Judge 固定、所有候选结果必须返回 main Agent，且任何 subagent 都不能直接进入 Core。
...
- 实现 Evidence Bundle v1、Record/Seal/Decision freshness、proof-impact、结果消费、Evidence 转义与正式
  Judgment 检查清单；
...
- 由现有 presentation adapter 在 accepted activation/revision 后写入简单 `interaction.md`，并实现 Route 设计时
  的窄只读规则；
- 更新窄 hooks；
- 完成 main-direct-work + required Judge 与 optional Driver/Verifier + required Judge 两条 Composition E2E，
  另保留 Core primitive E2E；
...
Agent-path optimization 按四个可独立观察的 release slice 验收：A=Proof Integrity，B=Risk-Adaptive
Delegation，C=C0 Agent Control，D0=Audit Contract/Experience。A-D0 都是 Phase 4 release claim 的组成部分；
D1 automated session adapter 只有第 9.1.3 节 trigger 全部满足后才进入范围，当前保持 `not_evaluated`。
...
| Evidence | Inline/CoreSnapshot freeze、接纳前态、批内唯一、Context、claims domain；Bundle v1 四 kind、131072-byte budget、deterministic canonical identity、coherence/current applicability、完整 mixed-baseline Packet |
...
| Composition | Record/Seal/Decision 三道 freshness、stale baseline 拒绝、proof-impact/Remap、实际 effect 核查、双 freeze 隔离、风险驱动 coverage、每 Stage Review 的 required Judge gate、正式 Judgment 归 main Agent |
...
| Agent context | 默认 SQL/mutation/skill/subagent 输出只含当前动作所需的最小 material；完整 digest 只在机器 identity/freeze 边界出现一次，重复上下文使用 local id 与末 7 位 display hint；无全量 Trail/大对象/整件 artifact/重复 fanout、CSV、refresh task 或 report log；仅 Route 设计读取至多一份 interaction；正式 Wait 超预算只返回 summary 并保持 Waiting |
...
- 不使用 Driver/Verifier、由 main Agent 执行工作并在 Review 使用 required Judge 的完整 loop；
- 使用任意模型外候选结果并经 main Agent 转义、随后使用 required Judge 的完整 loop；
...
- Evidence Bundle clean-host E2E 在 Record 前、Record 后/Seal 前、Seal 后/Decision 前分别注入 material drift，
  并从 Trail/projection 证明候选零提交、fresh Evidence 追加且旧 Evidence 保留、以及 stale accept 路径停止；
- proof-impact fixture 覆盖 lease timing/token、symlink containment、migration/bootstrap ownership、可证明无影响的
  current-Stage 文档、unknown fail-closed 和 terminal finding/new Objective；
- 在不安装 Python、virtualenv 或 pip、但提供受支持 SQLite CLI 的 clean environment 中完成两条带 required
  Judge 的 Composition E2E，并单独完成 Core primitive path；
...
- Composition 把依赖可变材料的 prose-only observation、无效/不一致/超预算 Bundle，或没有完整机器 digest 的
  Bundle 作为 `satisfied` 依据；
- Seal 前 drift 未在 Attempting 追加 fresh Evidence，或 Decision 前没有 current-applicable supports Evidence 仍
  进入 accept；
- 后续 effect 影响 accepted proof 却绕过 `RequestRemap`、显式 carry/reverification，或在终态重开旧 Objective；
...
- Agent 把末 7 位或其他短 digest 用于 equality、lookup、freeze、admission、artifact integrity 或 Core input；
- Composition 跳过任一 Stage Review 的 required Judge，或把 Judge 数量、投票、Runtime success、模型身份或
  disposition 当作自动 accept；Driver/Verifier 与额外 Judge 仍必须按 Evidence coverage、显式 verification rule
  和信息价值选择；
...
- C1 最小 corpus/trigger/accepted ADR 不满足却新增 inspect、generic query、MCP read tool 或第二正常 read path；
- D1 trigger 不满足却发布 automated session parser/renderer、保存 Runtime mirror，或让 session audit 成为 Core、
  Review、proof 或 completion 输入；
