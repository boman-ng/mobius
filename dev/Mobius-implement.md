...
3. 只有新提交并产生 `Achieved` 或 `Abandoned` 的 transition 才触发 terminal fanout；幂等 mutation 重放不触发。
...
普通 SQL read、mutation response、skill instruction 与 Subagent envelope 都不返回 view path、文件列表、CSV
...
| Human view | path containment、pinned heads、unique generation、正文 UTF-8/CSV 结构校验、精确历史 heads/digest stale detection、formula neutralization |
| Agent context | 默认 SQL/mutation/skill/subagent 输出只含当前动作所需的最小 material；无全量 Trail/大对象/整件 artifact/重复 fanout、view path、CSV、refresh task 或 report log；正式 Wait 超预算只返回 summary 并保持 Waiting |
...
- 默认 Agent-facing SQL、mutation、skill 或 Subagent result 注入 CSV 内容、view path、refresh task 或生成日志；
